...
| 前端框架 | React + Vite + TypeScript | 19 / 6 / ~5.6 |
| LLM | Cloudflare AI Gateway → Gemma 4 | gemma-4-31b-it |
...
│   └── web/                          # React 前端
│       ├── src/
│       │   ├── App.tsx               # 主应用 (模式选择→建档→交互)
│       │   ├── api/client.ts         # API 客户端 (全部后端调用)
│       │   ├── styles.css            # 全局样式
│       │   └── components/           # 22 个组件
│       │       ├── ModeSelectionLanding.tsx  # 模式选择首页
│       │       ├── ProfileOnboardingForm.tsx # 完整建档
│       │       ├── QuickProfileForm.tsx      # 快速建档
│       │       ├── RoundTableDebate.tsx      # 辩论可视化
│       │       ├── TodayDashboard.tsx        # 今日仪表盘
│       │       ├── AutoDraft.tsx             # 自动草稿
│       │       ├── BalanceMode.tsx           # 三维平衡
│       │       ├── ReminderCard.tsx          # 饭点提醒
│       │       ├── ActivePlanPanel.tsx       # 方案管理
│       │       ├── RecommendationCard.tsx    # 推荐卡片
│       │       ├── MetricsPanel.tsx          # 指标面板
│       │       ├── LearningPanel.tsx         # 学习进度
│       │       ├── NutritionBalancePanel.tsx # 营养平衡
│       │       ├── BudgetBar.tsx             # 预算条
│       │       ├── SafetyBanner.tsx          # 安全警告
│       │       ├── ProviderBadge.tsx         # 数据源标识
│       │       ├── OrderConfirmModal.tsx     # 下单确认
│       │       ├── ChatMessageList.tsx       # 聊天列表
│       │       ├── AgentDebatePanel.tsx      # Agent 辩论
│       │       ├── ProfileCard.tsx           # 档案卡片
...
SupervisorAgent
    │
    ├── OrchestratorAgent (关键词映射选 3-5 个 Agent)
    │       │
    │       ▼ asyncio.gather()
...
前端代理配置在 `apps/web/vite.config.ts`，API 请求自动代理到 `localhost:8001`。

## 环境变量

见 `.env.example`：

- `CF_AIG_TOKEN` / `CF_AIG_BASE_URL` / `GEMMA_MODEL` — LLM 配置
